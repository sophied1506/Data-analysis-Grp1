...
We start by fitting the full regression model containing all explanatory variables. Since our response variable is binary, we fit a logistic regression model without interaction as we want to focus onthe individual effects of each of our covariates. The model can be written as:
...
The AIC for this model is 17011.99, As this value is rather high, we employ stepwise regression with backward selection to see if we can achieve a better fit, based off of the lowest AIC. The new model we fit is defined as follows:
